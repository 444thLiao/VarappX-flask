...
def get_users_info(db='demo_mini', rank_required=ADMIN_LEVEL):
    from varappx.data_models.users import users_list_from_users_db
...
def get_dbs_info(db='demo_mini', rank_required=ADMIN_LEVEL):
    from varappx.data_models.users import databases_list_from_users_db
    if auto_process_OPTIONS(request):
        return auto_process_OPTIONS(request)
...
def get_roles_info(db='demo_mini', rank_required=ADMIN_LEVEL):
    from varappx.data_models.users import roles_list_from_users_db
    if auto_process_OPTIONS(request):
        return auto_process_OPTIONS(request)
...
def signup(email_to_file=None):
    ...
    """
    from varappx.main.view_tools import authenticate
    ...
    user, msg = authenticate.create_user(username, password, firstname, lastname, email, phone, email_to_file)
    if user is None:
        return abort(msg)
...
@main.route('/resetPasswordRequest', methods=['OPTIONS', 'POST','GET'])
@cors_handle
def reset_password_request(email_to_file=None):
...
@main.route('/changePassword', methods=['OPTIONS', 'POST','GET'])
@cors_handle
def change_password(new_password=None, email_to_file=None):
...
def change_attribute(user=None, rank_required=GUEST_LEVEL):
    ...
    if mod_user.username == username and mod_user.code == code:
...
@cors_handle
@permission_required
def delete_user(rank_required=ADMIN_LEVEL):
    from varappx.main.view_tools import authenticate as auth
...
@cors_handle
@permission_required
def attribute_db(user=None, rank_required=ADMIN_LEVEL):
    from varappx.main.view_tools import authenticate as auth
    if auto_process_OPTIONS(request):
        return auto_process_OPTIONS(request)
    username = request.form['username']
    ...
    user = auth.attribute_db(username, code, dbname, add)
    id_token = JWT_user(user, TOKEN_DURATION)
    return jsonify({'id_token': id_token})
