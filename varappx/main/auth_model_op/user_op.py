...
def user_factory(u):
    ...
    databases = []
    for _db in user_dbs:
        if not _db.name in [_vdb.name for _vdb in VariantsDb.query.all()]:
        ...
        # if not os.path.exists(db.app.DB_USERS):
        #     #logger.warning("Database '{}' not found on disk!".format(db.name))
        #     vdb = VariantsDb.objects.get(name=_db.name)
...
def database_factory(d):
    ...
    users = [acc.users.username for acc in DbAccess.query.filter_by(variantsdb=d)]
...
def users_list_from_users_db(query_set=None, db='default'):
    ...
    if query_set is None:
        query_set = Users.query.join('DbAccess', 'variantsdb').filter_by(name=db).all()
...
def databases_list_from_users_db(query_set=None, db='default'):
    ...
    if query_set is None:
        query_set = VariantsDb.query.filter_by(name=db).filter_by(is_active=1).all()
...
def roles_list_from_users_db(query_set=None, db='default'):
    ...
    if query_set is None:
        query_set = Roles.query.join('users','DbAccess','variantsdb').filter_by(name=db).all()
...
def persons_list_from_db(query_set=None, db='default'):
    ...
    if query_set is None:
        query_set = People.query.join('users','DbAccess','variantsdb').filter_by(name=db).all()
